...
button :: proc(info: Button_Info, loc := #caller_location) -> (res: Button_Result) {
	...
	text_info: Text_Info = {
		...
		size = 20,//core.style.button_text_size,
	}
	...
	widget.hover_time = animate(widget.hover_time, 0.25, .Hovered in widget.state)

	draw_rounded_box_fill(widget.box, core.style.rounding, blend_colors(widget.hover_time, core.style.color.substance, core.style.color.content))
	draw_text(center(widget.box) - text_size / 2, text_info, blend_colors(widget.hover_time, core.style.color.content, core.style.color.foreground))
