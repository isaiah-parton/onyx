...
icon_section :: proc(state: ^Icon_Section_State) {
	...
	if do_layout(on_top, left_to_right) {
		...
		shrink(get_current_style().scale * 1)
		set_padding(0)
	...
	if do_layout(right_to_left) {
		...
		set_size(to_layout_size)
		if begin_container(
			// space = [2]f32{f32(how_many_columns), f32(how_many_rows)} * icon_cell_size + 12,
		) {
			if container_object, ok := get_current_object(); ok {
				kn.add_box(container_object.box, get_current_style().rounding, paint = get_current_style().color.background)
				kn.add_box_lines(
					container_object.box,
					get_current_style().line_width,
					get_current_style().rounding,
					paint = get_current_style().color.lines,
				)
				...
				for i in 0..<how_many_rows {
					...
					if get_clip(current_clip(), get_current_layout().box) == .Full {
						continue
					}
					...
					for j in (i * how_many_columns)..<min((i + 1) * how_many_columns, how_many_icons) {
						...
						if object_is_visible(object) {
							...
							kn.add_box_lines(
								...
								get_current_style().line_width,
								get_current_style().rounding,
								kn.fade(get_current_style().color.lines, object.animation.hover),
							)
