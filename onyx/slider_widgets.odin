...
display_slider :: proc(info: Slider_Info($T)) -> (result: Slider_Result(T)) {
	...
	if widget.visible {
		...
		draw_rounded_box_fill({widget.box.low, {widget.box.low.x + box_width(widget.box) * time, widget.box.high.y}}, radius, core.style.color.accent)
		draw_arc_fill(widget.box.low + radius + {time * range_width, 0}, h / 2, 0, math.TAU, core.style.color.background)
		draw_arc_stroke(widget.box.low + radius + {time * range_width, 0}, h / 2, 0, math.TAU, 1.5, core.style.color.accent)
	}
