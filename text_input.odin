...
input_from_object :: proc(
	...
	if is_visible {
		...
		vgo.pop_scissor()
		if .Active in object.state.current {
			vgo.stroke_box(object.box, 1, radius = rounding, paint = style().color.accent)
		}
	}
}
...
raw_input :: proc(
	...
	if begin_object(object) {
		...
		if .Active in (object.state.current - object.state.previous) {
			strings.builder_reset(&extras.builder)
			strings.write_string(&extras.builder, content^)
		}

		input_from_object(object, extras, content^, prefix, placeholder, is_multiline, is_monospace)
		...
		if .Active in object.state.current && key_pressed(.Enter) {
