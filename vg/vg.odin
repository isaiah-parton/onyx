...
create_image_path :: proc(ctx: ^Context, filename: cstring, imageFlags: Image_Flags) -> int {
	...
	image := create_image_rgba(ctx, int(w), int(h), imageFlags, data)
	stbi.image_free(img)
	return image
}
...
create_image_mem :: proc(ctx: ^Context, data: []byte, imageFlags: Image_Flags) -> int {
	...
	image := create_image_rgba(ctx, int(w), int(h), imageFlags, pixel_data)
	stbi.image_free(img)
	return image
}
...
__allocTextAtlas :: proc(ctx: ^Context) -> bool {
	...
	if ctx.fontImages[ctx.fontImageIdx+1] != 0 {
		iw, ih = image_size(ctx, ctx.fontImages[ctx.fontImageIdx+1])
	} else { // calculate the new font image size and create it.
		iw, ih = image_size(ctx, ctx.fontImages[ctx.fontImageIdx])
