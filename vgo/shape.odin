...
make_arc :: proc(center: [2]f32, from, to, radius, width: f32) -> Shape {
	from, to := from, to
	// be nice
...
get_shape_bounding_box :: proc(shape: Shape) -> Box {
	...
	case .Path:
		for i in 0 ..< shape.count * 3 {
			j := shape.start + i
			box.lo = linalg.min(box.lo, core.gfx.cvs.data[j] - 2)
			box.hi = linalg.max(box.hi, core.gfx.cvs.data[j] + 2)
		}
	case .Polygon:
...
fill_box :: proc(box: Box, paint: Paint_Option, radius: [4]f32 = {}) {
	draw_shape(make_box_shape(box, radius), color)
}

stroke_box :: proc(box: Box, width: f32, color: Color) {
...
draw_rounded_box_corners_fill :: proc(box: Box, corners: [4]f32, color: Color) {
	...
	draw_shape(add_shape_box(box, corners), color)
}
...
draw_rounded_box_fill :: proc(box: Box, radius: f32, color: Color) {
	...
	draw_shape(add_shape_box(box, radius), color)
}
